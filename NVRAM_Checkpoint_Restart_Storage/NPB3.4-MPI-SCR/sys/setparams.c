...
{
  char line[LL];
  char mpifc[LL], flink[LL], fmpi_lib[LL], fmpi_inc[LL], fscr_lib[LL], fscr_inc[LL], fflags[LL], flinkflags[LL];
...
setparams: File %s doesn't exist. To build the NAS benchmarks\n\
  ...
  strcpy(fscr_lib, DEFAULT_MESSAGE);
  strcpy(fscr_inc, DEFAULT_MESSAGE);
  ...
  while (fgets(line, LL, deffile) != NULL) {
    ...
    check_line(line, "FSCR_LIB", fscr_lib);
    check_line(line, "FSCR_INC", fscr_inc);
  ...
  switch(type) {
      ...
      case CG:
          ...
          put_string(fp, "cs8", fscr_lib);
          put_string(fp, "cs9", fscr_inc);
          break;
      case IS:
      case DT:
