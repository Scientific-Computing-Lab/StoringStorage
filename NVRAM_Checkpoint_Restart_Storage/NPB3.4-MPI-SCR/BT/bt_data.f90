...
!---------------------------------------------------------------------
   ...
   &           BOTTOM=6000, TOP=7000)
    ...
    integer predecessor(3), successor(3)
    integer, allocatable ::  &
   &        cell_coord(:, :), cell_low(:, :),  &
   &        cell_high(:, :), cell_size(:, :),  &
   &        start(:, :), end(:, :),  &
...
!---------------------------------------------------------------------
   ...
   &        t_bpack, t_exch, t_xcomm, t_ycomm, t_zcomm, t_recover, &
   &        t_checkpoint, t_compute, t_last
    parameter(t_total=1, t_io=2, t_rhs=3, t_xsolve=4, t_ysolve=5,  &
   &        t_zsolve=6, t_bpack=7, t_exch=8, t_xcomm=9,  &
   &        t_ycomm=10, t_zcomm=11, t_recover=12, t_checkpoint=13, &
   &        t_compute=14, t_last=14)
    logical timeron
