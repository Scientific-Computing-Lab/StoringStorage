...
!---------------------------------------------------------------------
  ...
  &             ' totcomp', ' totcomm', 'recover', 'ckpt', 'compute'/
...
!---------------------------------------------------------------------
    ...
    call get_environment_variable('SCR_CHECKPOINT_DIR', scr_checkpoint_dir)
    call scr_have_restart(flag, ckptname, error)
    if (flag == 1) then
        print *, "Performing recovery"
        call mpi_barrier(comm_setup, error)
        call bt_scr_recover(error)
        call mpi_barrier(comm_setup, error) 
        step = step + 1
    end if
...
200             format(' Time step ', i4)
        ...
        call timer_start(t_compute)
        call adi
        call timer_stop(t_compute)
...
1100        format(/' BTIO -- statistics:'/  &
        ...
        write (*, 1110) timer_read(t_recover), timer_read(t_checkpoint), & 
                      & timer_read(t_compute)
1110    format(/' SCR -- statistics:'/  &
               &               '   Recovery time in seconds    : ', f14.2/ &
               &               '   Checkpoint time in seconds  : ', f14.2/ &
               &               '   Compute time in seconds     : ', f14.2)
...
999 continue
    ...
    call scr_finalize(error)  ! Finialize checkpoint restart before closing mpi.
